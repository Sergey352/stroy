...
class PyrusClient:
    ...
    def sync_catalog_schema(self, catalog_id, catalog_headers):
        """Меняет набор колонок УЖЕ существующего справочника — например,
        добавляет новую колонку. Метод POST /catalogs/{id} — сравни с
        POST /catalogs/{id}/diff (upsert/delete СТРОК по ключу, его
        используют catalogs_seed.py/smeta_catalog_import.py) — а это
        меняет саму СХЕМУ (список колонок).

        ВАЖНО (найдено эмпирически в сессии 2026-08-21, в документации
        описано без примера): без `"apply": true` запрос ничего не
        применяет — просто возвращает предпросмотр того, что получилось
        бы (в ответе будет `"apply": false`). Поэтому здесь apply
        жёстко зашит в true — вызывающий код должен быть уверен, что
        передаёт ПОЛНЫЙ список колонок (старые + новые), а не только
        добавляемые — API не умеет "добавить одну колонку", только
        целиком переопределить список.

        ОСТОРОЖНО: значения (items) в этом запросе не передаются
        (только заголовки) — но сам метод предназначен для полной
        синхронизации содержимого тоже, поэтому лучше применять его
        только на пустом справочнике (после diff-delete всех строк) —
        не проверялось, что происходит со старыми значениями строк при
        одновременном добавлении новой колонки на непустом справочнике."""
        return self.post(
            f"catalogs/{catalog_id}",
            {"apply": True, "catalog_headers": catalog_headers, "items": []},
        )
